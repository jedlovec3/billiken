---
title: "Billiken League Pre-Freeze Rankings 2026"
...
frozen_rosters <- read_sheet(sheet_id, sheet = "FrozenRosters", col_types = 'cccccc')
draft <- read_sheet(sheet_id, sheet = "Draft", col_types = 'ciiicccccc')
...
# Read positions from ESPN API (via fetch_espn_positions.R)
...
(replacement_level <- rbind(rl_c, rl_1b, rl_2b, rl_3b, rl_ss, rl_of, rl_ci, rl_mi, rl_util, rl_p))
...
```{r}
write_csv(par, "projections_2026.csv")
...
cut_last <- par %>% 
  filter(billikenTeam == "Free At Last") %>% 
  filter(row_number(desc(ev)) <= 15L)
...
projected_keepers <- rbind(cut_blue, cut_melon, cut_erie, cut_past, cut_bigred, cut_last, cut_birds, cut_west, cut_slug, cut_hoos)
