...
Set projections year (controls which local CSVs are loaded)
...
positions <- suppressWarnings(read_sheet("https://docs.google.com/spreadsheets/d/1ZjlBTRAnW8vTzdr4rY-ciQWmYMPvEF5xtGQkUNLg4a0/edit?gid=605162437#gid=605162437", sheet = "Positions", col_types = 'ciiiiiiiiiii') %>% 
    ...
    select(player, p_c, p_1b, p_2b, p_3b, p_ss, p_of, p_ci, p_mi, p_dh, p_util))
...
Combine frozen and drafted players
```{r}
...
Merge projections with rosters
```{r}
...
```{r}
ggplot(hitter_points, aes(AVG, avg_pct)) +
  geom_point() +
...
hitter_points$hr_pts_pred = predict(hr_model_glm, hitter_points, type="response")*9+1

r_model_glm <- glm(r_pct ~ R, data = hitter_points, family = "binomial")
hitter_points$r_pts_pred = predict(r_model_glm, hitter_points, type="response")*9+1

rbi_model_glm <- glm(rbi_pct ~ RBI, data = hitter_points, family = "binomial")
hitter_points$rbi_pts_pred = predict(rbi_model_glm, hitter_points, type="response")*9+1

sb_model_glm <- glm(sb_pct ~ SB, data = hitter_points, family = "binomial")
hitter_points$sb_pts_pred = predict(sb_model_glm, hitter_points, type="response")*9+1

avg_model_glm <- glm(avg_pct ~ AVG, data = hitter_points, family = "binomial")
hitter_points$avg_pts_pred = predict(hr_model_glm, hitter_points, type="response")*9+1
...
pitcher_points$w_pts_pred = predict(w_model_glm, pitcher_points, type="response")*9+1

sv_model_glm <- glm(sv_pct ~ SV, data = pitcher_points, family = "binomial")
pitcher_points$sv_pts_pred = predict(sv_model_glm, pitcher_points, type="response")*9+1

so_model_glm <- glm(so_pct ~ SO, data = pitcher_points, family = "binomial")
pitcher_points$so_pts_pred = predict(so_model_glm, pitcher_points, type="response")*9+1

era_model_glm <- glm(era_pct ~ ERA, data = pitcher_points, family = "binomial")
pitcher_points$era_pts_pred = predict(era_model_glm, pitcher_points, type="response")*9+1

whip_model_glm <- glm(whip_pct ~ WHIP, data = pitcher_points, family = "binomial")
pitcher_points$whip_pts_pred = predict(whip_model_glm, pitcher_points, type="response")*9+1
...
projected_players <- bind_rows(hitter_projections, pitcher_projections) %>% 
  ...
  select(Name, billikenTeam, contract, salary, Team, PA, AB, H, HR, R, RBI, SB, AVG, IP, WH, ER, W, SV, SO, ERA, WHIP, point_value, p_c, p_1b, p_2b, p_3b, p_ss, p_of, p_ci, p_mi, p_dh, p_util) %>% 
  arrange(desc(point_value)) #%>% 
  #filter(billikenTeam == "Melonheads") 
...
par <- projected_players %>% 
  ...
  arrange(desc(par)) %>% 
  select(Name, Team, billikenTeam, contract, salary, point_value, repl, par, PA, AB, H, HR, R, RBI, SB, AVG, IP, ER, WH, H, W, SV, SO, ERA, WHIP, p_c, p_1b, p_2b, p_3b, p_ss, p_of, p_ci, p_mi, p_dh, p_util)
...
```{r}
ggplot(draft_eligible, aes(x = pick, y = ev)) + 
